"""Manage RADOS pools through the Ceph Dashboard REST API."""
...
def list_(attrs=None, stats=False, profile="default"):
    """List RADOS pools with optional selected attributes and statistics.
...
def get(pool_name, attrs=None, stats=False, profile="default"):
    """Return one RADOS pool and its RBD configuration.
...
def create(
    ...
    .. code-block:: bash
        ...
        salt-call --local ceph_pool.create volumes 32 replicated \\
          ...
          options='{"size":3,"pg_autoscale_mode":"on"}'
    """
    return _invoke(
        pool_api.create,
        pool_name,
...
def update(
    ...
    """Update mutable pool values, metadata, configuration, or name.
    ...
    .. code-block:: bash
        ...
        salt-call --local ceph_pool.update volumes \\
          application_metadata='["rbd"]' \\
          options='{"size":3,"quota_max_bytes":1073741824}'
    """
    return _invoke(
        pool_api.update,
        pool_name,
