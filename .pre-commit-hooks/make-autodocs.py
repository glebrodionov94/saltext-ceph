...
src_dir = repo_path / "src" / "saltext" / "ceph"
...
"""
    if not rst_path.exists() or rst_path.read_text() != module_contents:
        print(rst_path)
        rst_path.write_text(module_contents, encoding="utf-8", newline="\n")
        return True
    return False
...
def write_index(index_rst, import_paths, kind):
    if kind == "utils":
        ...
        common_path = posixpath.commonpath(
            tuple(x.replace(".", "/") for x in import_paths)
        ).replace("/", ".")
...
"""
    if not index_rst.exists() or index_rst.read_text() != index_contents:
        print(index_rst)
        index_rst.write_text(index_contents, encoding="utf-8", newline="\n")
        return True
    return False
