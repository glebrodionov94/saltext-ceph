...
# Salt 3008 pulls in timelib, which needs an MSVC toolchain on Windows.
# Keep local Windows lint/test setup usable without a native compiler.
# CI explicitly selects each supported Salt version through SALT_REQUIREMENT.
SALT_REQUIREMENT = os.environ.get("SALT_REQUIREMENT") or (
    "salt==3006.27" if sys.platform == "win32" else "salt>=3006"
)
    ...
    *passed_requirements,  # pylint: disable=unused-argument
    ...
    if SKIP_REQUIREMENTS_INSTALL is False:
        ...
        if install_source:
            ...
            session.install("-e", pkg, SALT_REQUIREMENT, silent=PIP_INSTALL_SILENT)
        elif install_extras:
            pkg = f".[{','.join(install_extras)}]"
            session.install(pkg, SALT_REQUIREMENT, silent=PIP_INSTALL_SILENT)
...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=src/saltext/ceph/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/ceph/*")
            ...
            #    "--include=src/saltext/ceph/*,tests/*"
            # )
        finally:
